...
import { Button, Card, Descriptions, Empty, Form, Image, Input, Modal, Select, Table, Tag, Typography, message } from 'antd';
...
export default function AssetDetailsPage() {
  ...
  const historyColumns = [
    ...
    { title: 'Kimdan', render: (row) => row.fromUser?.fullName || '—' }, { title: 'Kimga', render: (row) => row.user?.fullName || '—' },
    { title: 'Yo‘nalish', render: (row) => <span>{row.fromDepartment?.name || '—'} <span className="history-arrow">→</span> {row.toDepartment?.name || '—'}</span> },
    { title: 'Sana va vaqt', dataIndex: 'createdAt', width: 180, render: (date) => new Date(date).toLocaleString('uz-UZ') }, { title: 'Amalni bajargan', render: (row) => row.actor?.fullName || '—' }
  ];
