...
export const createAsset = async (req,res,next)=>{try{res.status(201).json(await assetService.create(req.body, req.user.id));}catch(e){next(e)}};
export const updateAsset = async (req,res,next)=>{try{res.json(await assetService.update(req.params.id, req.body, req.user.id));}catch(e){next(e)}};
export const deleteAsset = async (req,res,next)=>{try{await assetService.remove(req.params.id, req.user.id);res.status(204).end();}catch(e){next(e)}};
