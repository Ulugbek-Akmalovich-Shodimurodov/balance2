...
async function main() {
  ...
  const admin = await prisma.user.upsert({ where: { email: 'admin@example.com' }, update: { login: 'admin' }, create: { fullName: 'Administrator', email: 'admin@example.com', login: 'admin', password, role: 'ADMIN' } });
